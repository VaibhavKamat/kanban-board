...
def test_init_db_migrates_old_style_cards_table(tmp_path, monkeypatch):
    db_path = tmp_path / "test.db"
    monkeypatch.setattr(db, "DB_PATH", db_path)

    # Simulate a database created before cards.due_date existed.
    conn = sqlite3.connect(db_path)
    conn.execute(
        "CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT NOT NULL UNIQUE, "
        "email TEXT, password_hash TEXT)"
    ...
    conn.execute(
        "INSERT INTO users (username, email, password_hash) VALUES ('user', 'user@example.com', 'x')"
    )
    conn.execute(
        "CREATE TABLE boards (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, "
        "type TEXT NOT NULL DEFAULT 'personal', name TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)"
    )
    conn.execute("INSERT INTO boards (user_id) VALUES (1)")
    conn.execute(
        "CREATE TABLE columns (id INTEGER PRIMARY KEY AUTOINCREMENT, board_id INTEGER NOT NULL, "
    ...
    conn = db.get_connection()
    try:
        card = conn.execute("SELECT title, due_date FROM cards WHERE id = 1").fetchone()
    finally:
        conn.close()
